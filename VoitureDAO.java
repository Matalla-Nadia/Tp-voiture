...
/* Read all---------------------------------------------------------------------------------------------------------------------------------------*/
 ...
 /* read by type---------------------------------------------------------------------------------------------------------------*/	
		public ArrayList<Voitures> readByType(String id) {
			ArrayList<Voitures> response = null;
			try {
				PreparedStatement request = connect.prepareStatement("SELECT v.id, v.nom, v.description,v.image,v.prix, type.id, type.nom FROM voitures v, type t "
																	+ "where v.type= t.id and t.id=" + id);
				ResultSet rs = request.executeQuery();
				response = new ArrayList<Voitures>();
				while(rs.next()) {
